...
class ResNet18Cfix(nn.Module):
    def __init__(self, feature_dim, num_classes, pseudo_dim, arch='', f_extraction=False, self_supervised=False, path_model=None):
        ...
        resnet = self.get_backbone(self.path_model)
        if self_supervised:
            self.f = resnet
        else:
            self.conv1 = resnet.conv1
    ...
    def get_backbone(self, path_model):
