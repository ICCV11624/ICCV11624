...
class CfixTrainer():
    def __init__(self, args, model, loaders, optimizer, num_classes, t, beta):
        self.args = args
        self.model = model 
    ...
    def _extract_features(self, loaders):
        ...
        if self.args['dataset'] == 'celebA':
            if self.args['self_supervised']:
                r_self = '/mnt/beegfs/work/H2020DeciderFicarra/gcapitani/Barlow_twins/celebA/lightning_logs/version_0'
            else:
                r_self = '/mnt/beegfs/work/H2020DeciderFicarra/gcapitani/Imagenet/celebA/lightning_logs/version_0'
        ...
        features['train_eval'] = torch.load(os.path.join(r_self,'train_eval', 'tensor.pt'), map_location=self.device)
        features['train_eval'] = torch.nn.functional.normalize(features['train_eval'])

        features['valid'] = torch.load(os.path.join(r_self,'valid', 'tensor.pt'), map_location=self.device)
        features['valid'] = torch.nn.functional.normalize(features['valid'])

        features['test'] = torch.load(os.path.join(r_self,'test', 'tensor.pt'), map_location=self.device)
